	...
	return (
		...
		<div className='px-5 pt-3'>
            ...
            <div className="d-flex gap-0 column-gap-3 overflow-auto">
                {store.pokemon.map((item) => (
                    <div className="col p-5" key={item.uid}>
                        <div className="card p-2 g-col-6" style={{ width: "250px" }}>
                            <img src={`https://pokeapi.co/api/v2/pokemon/${item.uid}.jpg`} className="card-img-top" alt="..." />
                            <div className="card-body p-7">
                                <h5 className="card-title text-center">{item.name}</h5>
                                <Link className='btn btn-outline-info mx-2' to={`/pokemon/${item.uid}`}>Learn more!</Link>
